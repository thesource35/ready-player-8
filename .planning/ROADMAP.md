...
| 999.5 | Production-readiness sweep before public launch (millions-of-users bar) | 2026-04-27 | User-stated bar: app will serve millions of users; everything must be perfect. Surfaced from end-of-Phase-30.1-UAT discussion 2026-04-27. Captures gaps that are individually small but collectively block "production-ready" claim: **(a) Phase 30.1 UAT Step 5** — cross-launch Keychain persistence not directly verified (de-scoped as Phase 29.1 invariant for AUTH-GATE-04 closure, but at scale a 0.01% session-restore failure = thousands of broken accounts/day, so should be actually tested). **(b) `validateBaseURL` accepts URLs with paths beyond the host** (e.g., trailing `/rest/v1/`) — caused the 2026-04-27 UAT to hit a PGRST125 trap; fix by stripping/rejecting non-empty paths. **(c) Green-checkmark-not-tappable trap** — once `isConfigured == true`, Configure Backend affordance disappears; if user saved bad creds that pass `validateBaseURL` but fail at runtime, only recovery is uninstall (App Store users can't easily uninstall+reinstall). Make checkmark tappable to re-open the sheet. **(d) Silent demo-data fallbacks** — Tier 1 (visible "DEMO MODE" banner), Tier 2 (no fallback when configured), Tier 3 (compile-time DTO drift safety + bundle gating). **(e) Test target compile-only verification** — `xcodebuild test` blocked by pre-existing async errors in `ready_player_8Tests.swift`; resolve so Swift Testing suites actually run on CI. **(f) Email delivery smoke test** — `uuid v14` override applied 2026-04-26 closes the moderate alert but svix/resend hot path was not exercised at runtime; send a test portal invite to confirm hot path still works. **(g) `xlsx` SheetJS CDN tarball** — Vercel deploy-time install needs first-deploy verification (CI environments may reject non-npm tarball URLs). **(h) 14 pre-existing uncommitted files in working tree** (Phase 19/21 plans, STATE.md, project.pbxproj, supabase/.temp, Phase 30 attach safety SQL, Phase 30 UAT log) — triage what should be committed vs reverted. **(i) DB integrity recovery from 999.4** — blocking for Phase 29 features. **(j) APNs real-device delivery test** for NOTIF-05 — physical iPhone + Apple Developer Portal Push Notifications capability toggle. **(k) Web `notifications.ts` proper client/server split** (Option B from 2026-04-26 boundary fix; current state is surgical single-file extraction, architectural cleanup deserves its own PR). Promote via `/gsd-new-milestone` when scoping — this is a milestone-level effort, not a single phase. Each item (a-k) can be its own phase. Recommended ordering: (i) DB integrity → (e) test target → (b)+(c) AuthGate hardening → (a) Step 5 verification → (j) APNs real-device → (d) demo-data tiers → (f)+(g) email/xlsx smoke → (h) working-tree triage → (k) notifications.ts cleanup. |
