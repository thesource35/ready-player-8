...
| 999.4 | Supabase remote DB integrity recovery (constructionos / `nzdbphddnrfybwecvsvq`) | 2026-04-27 | Surfaced during a `supabase db push --linked` attempt to apply 5 missing Phase 29 + Phase 21 migrations (`20260420001..20260420004` + `20260421001`). Two findings: **(1) Migration history vs schema reality mismatch.** `20260415002_phase22_video_assets` is recorded as applied in the remote `schema_migrations` table, but the actual `cs_video_assets` table does not exist (`relation "cs_video_assets" does not exist (SQLSTATE 42P01)` when `20260420001_phase29_cs_live_suggestions` tries to reference it via FK). Most likely cause: schema modified outside of migrations (manual drops/restore) or partial silent migration failure recorded as success. **(2) Two timestamp tag mismatches** — bare `20260406` and `20260407` in remote history that don't match local file timestamps; the `20260407` entry was repaired via `supabase migration repair --status reverted 20260407 --linked` during 2026-04-27 investigation, and the bare `20260406` was implicitly reconciled when `20260406_documents.sql` re-applied as no-op. **Partial progress:** `20260406_documents.sql` + `20260407_phase14_notifications.sql` applied successfully (idempotent no-ops). 5 Phase 29/21 migrations still pending. **Recovery path:** needs Docker available locally for `supabase db dump --linked --schema public` to introspect actual remote schema, then a targeted plan to either (a) re-apply Phase 22 video table migrations against the gap, or (b) write reverse + replay migrations to reconcile metadata. Promote via `/gsd-add-phase` when scoped — DO NOT run `supabase db reset --linked` without confirming this DB has no production user data (it would drop everything). Investigate before next push attempt against this project. |
