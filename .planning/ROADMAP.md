...
| 999.6 | iOS volatile-state data-loss bug — `loadJSON` without matching `saveJSON` | 2026-04-27 | **Real production data-loss bug surfaced 2026-04-27 during 999.5 audit.** Six iOS view files load persisted state on `.onAppear` (via `loadJSON("ConstructOS.X.Y", default: ...)`) but have **zero matching `saveJSON` calls**. Result: any user-added entity (submittal, project, contract, opportunity, etc.) is held in `@State` only, displayed correctly in the current session, then silently lost on next app launch (`.onAppear` reloads from UserDefaults which has nothing → falls back to hardcoded seed array → user's data is gone). **Affected files (loads / saves):** `OperationsCommercial.swift` (5/0 — submittals, contracts, project accounts, etc.), `MoneyLensView.swift` (1/0 — Wealth Suite), `OpportunityFilterView.swift` (2/0 — Wealth Suite), `PowerThinkingView.swift` (2/0 — Wealth Suite), `SatelliteRoofEstimatorView.swift` (2/0), `SecurityAccessView.swift` (1/0). **13 distinct load paths total**. **Fix:** for each `@State var X = loadJSON(...)`, identify every mutation site (.append, indexed write, removal) and add `saveJSON("ConstructOS.X.Y", X)` immediately after. Could batch via a `Persisted<T>` property wrapper or a SwiftUI `.onChange(of: X)` reactive save. **Severity:** HIGH at scale — silent data loss is the worst class of bug; users blame themselves or assume the app is broken. Should be fixed before any public launch. **Recommended phase scope:** ~1 day per file with mutation-site audit + write tests. **Detection method (reusable):** `for f in *.swift; do load=$(grep -c loadJSON $f); save=$(grep -c saveJSON $f); [ $load -gt 0 ] && [ $save -eq 0 ] && echo $f; done` |
