...
export async function GET(req: Request) {
  ...
  if (result.state === "unconfigured" && page === 0) {
    return NextResponse.json({ data: MOCK_CONTRACTS, hasMore: false, total: MOCK_CONTRACTS.length, demoMode: true });
  }
  if (result.state === "error") {
    return NextResponse.json({ error: "Failed to load contracts", data: [], hasMore: false, total: 0 }, { status: 500 });
  }
