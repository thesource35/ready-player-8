...
export async function GET(req: Request) {
  ...
  if (result.state === "unconfigured" && page === 0) {
    return NextResponse.json({ data: MOCK_PUNCH_ITEMS, hasMore: false, total: MOCK_PUNCH_ITEMS.length, demoMode: true });
  }
  if (result.state === "error") {
    return NextResponse.json({ error: "Failed to load punch list", data: [], hasMore: false, total: 0 }, { status: 500 });
  }
